...
async function main() {
  ...
  console.log(initReservesInputObjectArray.slice(6, 7));

  const PoolProxy = new ethers.Contract(
    contractAddresses.PoolProxy,
    abi.Pool,
    signer
  );
  console.log(await PoolProxy.getReservesList());

  const array = await PoolProxy.getReservesList();
  for (let i = 0; i < array.length; i++) {
    const token = new ethers.Contract(array[i], abi.MockToken, signer);
    console.log(await token.symbol());
  }

  // await PoolConfiguratorProxy.setPoolPause(false);

  // console.log(await PoolConfiguratorProxy.CONFIGURATOR_REVISION());

  // tx = await PoolConfiguratorProxy.initReserves(
  //   initReservesInputObjectArray.slice(6, 7)
  // );
  // await tx.wait(3);
