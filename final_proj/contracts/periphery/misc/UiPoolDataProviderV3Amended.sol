...
contract UiPoolDataProviderV3Amended is IUiPoolDataProviderV3 {
    ...
    {
        IAaveOracle oracle = IAaveOracle(provider.getPriceOracle()); //OK
        IPool pool = IPool(provider.getPool()); //OK
        AaveProtocolDataProvider poolDataProvider = AaveProtocolDataProvider(
            provider.getPoolDataProvider()
        ); //OK

        address[] memory reserves = pool.getReservesList(); //OK
        AggregatedReserveData[]
            memory reservesData = new AggregatedReserveData[](reserves.length); //OK
