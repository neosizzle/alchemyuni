...
contract ProtocolOwnedDEXLiquidity is Ownable {
    ...
    IERC20 public constant gFTM =
        IERC20(0x39B3bd37208CBaDE74D0fcBDBb12D606295b430a);
    ...
    function protocolOwnedReserves()
        public
        view
        returns (uint256 wftm, uint256 geist)
    {
        (uint reserve0, uint reserve1, ) = lpToken.getReserves();
        uint balance = lpToken.balanceOf(address(this));
        uint totalSupply = lpToken.totalSupply();
        return (
            reserve0.mul(balance).div(totalSupply),
            reserve1.mul(balance).div(totalSupply)
        );
    }
    ...
    function buyFTM(uint256 _amount) public {
        require(
            _amount <= availableForUser(msg.sender),
    ...
    function superPODL(uint256 _amount) public {
        require(
            treasury.lockedBalances(msg.sender) >= minSuperPODLLock,
            "Need to lock GEIST!"
        );
