...
async function main() {
  ...
  let tx;
  tx = await ACLManager.addPoolAdmin(await signer.getAddress());
  await tx.wait(6);
  tx = await ACLManager.addEmergencyAdmin(await signer.getAddress());
  await tx.wait(6);
  tx = await ACLManager.addAssetListingAdmin(await signer.getAddress());
  await tx.wait(6);
  console.log(
