...
async function main() {
  ...
  /* 2. Update FlashLoanPremium on PoolConfiguratorProxy */
  ...
  tx = await PoolConfiguratorProxy.updateFlashloanPremiumTotal(5);
  await tx.wait(6);

  tx = await PoolConfiguratorProxy.updateFlashloanPremiumToProtocol(4);
  await tx.wait(6);

  console.log(
    `\nSuccessfully updated FlashLoanPremium on PoolAddressesProvider\n`
  );
  ...
  if (isSupportedNetwork(currentChainID)) {
    await verify(await PoolAddressesProvider.getPool(), [
      contractAddresses.PoolAddressesProvider,
    ]);

    await verify(await PoolAddressesProvider.getPoolConfigurator(), [
      contractAddresses.PoolAddressesProvider,
    ]);
  } else {
    console.log(
